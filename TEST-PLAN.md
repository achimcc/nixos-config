...
## Root Cause
**Hostname Mismatch**: `flake.nix` definierte `nixosConfigurations.nixos`, aber `networking.hostName = "nixos"` → nixos-rebuild konnte Konfiguration nicht finden.

## Applied Fixes

### 1. Critical: Fixed flake.nix configuration name
- Changed: `nixosConfigurations.nixos` → `nixosConfigurations.nixos`
